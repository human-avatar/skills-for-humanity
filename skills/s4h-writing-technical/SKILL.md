...
- Pairs with `/s4h-writing-audience-calibration` — audience profile is the anchor for every other decision; the calibration audit belongs here.
- Pairs with `/s4h-writing-restructure` — documentation structure often needs reordering, particularly moving prerequisites before the steps that need them.
- Pairs with `/s4h-writing-line-editing` — technical writing often suffers from passive voice and nominalisation; line editing is often the final pass.
...
- **Header:** "Next"
- **Options:**
  - `/s4h-writing-audience-calibration` — Calibrate technical level for the audience
  - `/s4h-communication-clarity-audit` — Audit technical clarity throughout
  - `/s4h-writing-restructure` — Restructure technical content for clearer flow
