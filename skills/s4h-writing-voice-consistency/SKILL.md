---
name: s4h-writing-voice-consistency
...
- Pairs with `/s4h-writing-tone-alignment` — tone is one dimension of voice. Voice consistency addresses the full identity of the speaker; tone alignment addresses one specific dimension (register).
- Pairs with `/s4h-writing-audience-calibration` — voice must be calibrated to the audience; a perfectly consistent voice that is calibrated to the wrong reader is still failing.
- Pairs with `/s4h-writing-prose-elevation` — elevation must stay in voice; an elevated passage that sounds like a different writer has traded one problem for another.
...
- **Header:** "Next"
- **Options:**
  - `/s4h-writing-line-editing` — Edit for consistent voice throughout
  - `/s4h-writing-tone-alignment` — Align tone with the voice
  - `/s4h-writing-prose-elevation` — Elevate the consistent voice
